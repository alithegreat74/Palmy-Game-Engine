...
#error Palmy Only Supports Windows 
	...
	void WindowsWindow::Update()
	{
		...
		for (int i = -5; i < 6; i++)
		{
			for (int j = -5; j < 6; j++)
			{
				auto color = index % 3 == 0 ? glm::vec4(1.0f) : index % 3 == 1 ? glm::vec4(0.0f,0.0f,1.0f,1.0f) : glm::vec4(1.0f,0.0f,0.0f,1.0f);
				m_Renderer->RenderQuad(
					Transform2D(glm::vec2(((float)j) / 10, ((float)i) / 10), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.05f, 0.05f, 0.05f)), color);
				index++;
			}
		}
